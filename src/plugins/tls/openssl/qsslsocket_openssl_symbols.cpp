...
# if defined(Q_OS_UNIX)
    ...
    libcrypto->setFileNameAndVersion("/usr/lib/libcrypto"_L1, SHLIB_VERSION_NUMBER ""_L1);
...
# else
    libssl->setFileNameAndVersion("/usr/lib/libssl"_L1, -1);
    libcrypto->setFileNameAndVersion("/usr/lib/libcrypto"_L1, -1);
# endif
    if (libcrypto->load() && libssl->load()) {
